...
import { Search, BarChart3, FileText, Users, ShoppingCart, ClipboardList, Package, UtensilsCrossed, LayoutDashboard, Building } from 'lucide-react';
...
export function POSSystem() {
  ...
  const showAdminFeatures = user.rol === 'admin' || user.rol === 'gerente' || user.rol === 'cajero';

  // --- Estado Local del Componente ---
  const [cart, setCart] = useState<CartItem[]>([]); // Carrito de compras actual
  const [sales, setSales] = useState<Sale[]>([]); // Historial de ventas (puede ser hidratado desde el backend si es necesario)
  ...
  const { data: categories = [], isLoading: isLoadingCategories } = useQuery<Category[]>({
    ...
    staleTime: Infinity, // Las categorías rara vez cambian, por lo que las hacemos "eternamente frescas"
  });
  ...
  const confirmSale = useCallback(
    async (paymentMethod: string, invoiceData?: any) => {
      ...
      const newSale: Sale = {
        ...
        paymentMethod,
        timestamp: new Date(),
        cashier: user?.username || 'Desconocido', // Asegúrate de que `user` no sea nulo
        branch: user?.sucursal?.nombre || selectedBranch?.nombre || 'Desconocido',
      ...
      try {
        ...
        const backendResponse = await createSale({
          items: cart.map((item) => ({
            id: item.originalId || item.id, // Usar el ID original del producto para el backend
            quantity: item.quantity,
            price: item.price,
            notes: item.notes, // Asegúrate de enviar las notas del producto
          })),
          total: newSale.total,
          paymentMethod,
        ...
        try {
          if (user?.rol === 'admin' || user?.rol === 'gerente' || user?.rol === 'super_admin') {
    ...
    [cart, user, toast, queryClient, mesaNumero, tipoServicio]
  );
  ...
  const handleEditSale = useCallback(
    (editedSale: Sale) => {
      setSales((currentSales) => currentSales.map((sale) => (sale.id === editedSale.id ? editedSale : sale)));
      toast({
  ...
  // useEffect para hidratar ventas del cajero desde el backend al abrir la pestaña 'sales'
  useEffect(() => {
    if (activeTab === 'sales' && user?.username) {
      getVentasOrdenadas(100).then((ventas) => {
        // Filtrar solo las ventas del cajero logueado
        const ventasCajero = ventas.filter((v: any) => {
          // v.vendedor_nombre puede variar según el backend, ajustar si es necesario
          return (
        ...
        // Mapear al formato Sale si es necesario
        setSales(
          ventasCajero.map((v: any) => ({
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex flex-col">
      ...
      <nav className="border-b bg-white px-6 shadow-sm">
        <div className="flex flex-row gap-2">
          <button
            className={`px-4 py-3 rounded-t-lg font-semibold flex items-center gap-2 transition-all duration-200 ${
              activeTab === 'pos'
          ...
          {(user.rol === 'admin' || user.rol === 'gerente' || user.rol === 'cajero') && (
            <button
              className={`px-4 py-3 rounded-t-lg font-semibold flex items-center gap-2 transition-all duration-200 ${
                activeTab === 'sales'
                  ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-md'
        ...
        <nav className="border-b bg-gray-50 px-6 py-2 shadow-inner">
          <div className="flex flex-row gap-2">
            <button
              className={`px-4 py-2 rounded-md font-medium flex items-center gap-2 transition-all duration-200 ${
                activeDashboardSubTab === 'summary'
                  ? 'bg-white text-blue-700 shadow-sm border-b-2 border-blue-500'
            ...
            {(user.rol === 'admin' || user.rol === 'gerente') && (
              <button
                className={`px-4 py-2 rounded-md font-medium flex items-center gap-2 transition-all duration-200 ${
                  activeDashboardSubTab === 'sucursales'
                    ? 'bg-white text-blue-700 shadow-sm border-b-2 border-blue-500'
                    : 'hover:bg-gray-100 text-gray-600'
                }`}
      ...
      <main className="flex flex-1 p-6 gap-6 overflow-hidden">
        ...
        <div className="flex-1 overflow-auto pr-2">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    className="pl-10 h-12 text-base shadow-sm focus:ring-blue-500 focus:border-blue-500 transition-all"
                  />
                </div>
          ...
          {activeTab === 'sales' && (user.rol === 'admin' || user.rol === 'gerente' || user.rol === 'cajero') && (
            <SalesHistory
              sales={sales}
          ...
          {activeTab === 'dashboard' && showAdminFeatures && (
            <div className="space-y-8 w-full"> {/* Añadir w-full para que ocupe todo el ancho */}
              ...
              {activeDashboardSubTab === 'sucursales' && (user.rol === 'admin' || user.rol === 'gerente') && (
        ...
        {/* Panel del Carrito de Compras y Opciones de Servicio */}
