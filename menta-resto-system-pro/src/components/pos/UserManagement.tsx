...
import { getUsers, createUser, getBranches } from '@/services/api';
...
interface User {
  ...
  username: string;
  email: string;
  rol: 'cajero' | 'gerente' | 'admin' | 'cocinero';
  activo: boolean;
  created_at: string;
  id_sucursal: number;
...
export function UserManagement() {
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    password: '',
    rol: 'cajero' as 'cajero' | 'gerente' | 'admin' | 'cocinero',
    activo: true,
  ...
  const handleDeleteUser = (userId: string) => {
    ...
    if (user.rol === 'admin' && users.filter(u => u.rol === 'admin').length === 1) {
      if (user?.rol === 'admin') {
        toast({
          title: "Error",
    ...
    // Aquí deberías hacer la mutación para eliminar usuario en backend.
    // Por ahora, solo mostramos toast y recargamos lista:
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      ...
      case 'gerente': return <UserCheck className="h-4 w-4" />;
              ...
              <DialogContent className="max-w-md">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Select value={formData.rol} onValueChange={(value: 'cajero' | 'gerente' | 'admin' | 'cocinero') => setFormData({...formData, rol: value})}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="flex justify-end gap-2">
                    <Button variant="secondary" onClick={() => setIsAddDialogOpen(false)}>Cancelar</Button>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select
                value={formData.rol}
                onValueChange={(value: 'cajero' | 'gerente' | 'admin' | 'cocinero') => setFormData({...formData, rol: value})}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
