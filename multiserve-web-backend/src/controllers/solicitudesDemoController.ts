...
import { ApiResponse, PaginatedResponse, SolicitudDemo, SolicitudDemoCreate, SolicitudDemoUpdate } from '../types/index.js';
import { solicitudDemoSchema } from '../validators/index.js';

// Obtener todas las solicitudes de demo con paginación
export const getSolicitudesDemo = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const response: PaginatedResponse<SolicitudDemo> = {
      success: true,
    ...
    logError('Error obteniendo solicitudes de demo', error as Error);
...
export const getSolicitudDemoById = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    
    const result = await query(
      'SELECT * FROM solicitudes_demo WHERE id_solicitud = $1',
      [id]
    );
    ...
    const response: ApiResponse<SolicitudDemo> = {
      success: true,
    ...
    logError('Error obteniendo solicitud de demo por ID', error as Error);
    const response: ApiResponse = {
      success: false,
      message: 'Error interno del servidor',
      error: 'INTERNAL_SERVER_ERROR',
...
export const createSolicitudDemo = async (req: Request, res: Response): Promise<void> => {
  try {
    const solicitudData: SolicitudDemoCreate = req.body;
    ...
    logError('Error creando solicitud de demo', error as Error);
    const response: ApiResponse = {
      success: false,
      message: 'Error interno del servidor',
      error: 'INTERNAL_SERVER_ERROR',
...
export const updateSolicitudDemo = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const updateData: SolicitudDemoUpdate = req.body;
    ...
    logError('Error actualizando solicitud de demo', error as Error);
    const response: ApiResponse = {
      success: false,
      message: 'Error interno del servidor',
      error: 'INTERNAL_SERVER_ERROR',
...
export const deleteSolicitudDemo = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    await query(
      'DELETE FROM solicitudes_demo WHERE id_solicitud = $1',
      [id]
    );
    ...
    logError('Error eliminando solicitud de demo', error as Error);
    const response: ApiResponse = {
      success: false,
      message: 'Error interno del servidor',
      error: 'INTERNAL_SERVER_ERROR',
...
export const getSolicitudesDemoStats = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logError('Error obteniendo estadísticas de solicitudes de demo', error as Error);
    const response: ApiResponse = {
      success: false,
      message: 'Error interno del servidor',
      error: 'INTERNAL_SERVER_ERROR',
