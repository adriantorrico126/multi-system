...
export const solicitudDemoSchema = Joi.object<SolicitudDemoCreate>({
  nombre: Joi.string()
    .min(2)
    .max(100)
    .required()
...
export const conversionEventSchema = Joi.object<ConversionEventCreate>({
...
export const userSessionSchema = Joi.object<UserSessionCreate>({
  session_id: Joi.string()
    .uuid()
    .required()
    .messages({
...
export const newsletterSuscriptorSchema = Joi.object<NewsletterSuscriptorCreate>({
  email: Joi.string()
    .email()
    .max(100)
    .required()
...
export const validateData = <T>(schema: Joi.ObjectSchema<T>, data: any): { isValid: boolean; data?: T; errors?: string[] } => {
