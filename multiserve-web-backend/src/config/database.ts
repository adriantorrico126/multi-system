...
export const testConnection = async (): Promise<boolean> => {
  try {
    ...
    logError('❌ Error conectando a la base de datos', error as Error);
    return false;
  }
};
...
export const query = async (text: string, params?: any[]): Promise<any> => {
  ...
  try {
    ...
    const duration = Date.now() - start;
    logError('Error ejecutando query', error as Error, {
      query: text.substring(0, 100) + (text.length > 100 ? '...' : ''),
      duration: `${duration}ms`,
      params,
    });
...
export const initializeWebTables = async (): Promise<void> => {
  try {
    ...
    // Tabla de newsletter
    await query(`
      DROP TABLE IF EXISTS newsletter_suscriptores CASCADE;
    `);
    ...
    logError('❌ Error inicializando tablas de la página web', error as Error);
    throw error;
  }
};
