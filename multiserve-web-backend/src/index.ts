...
import { logInfo, logError, requestLogger } from './config/logger.js';
...
╚══════════════════════════════════════════════════════════════╝
    ...
    logError('❌ Error inicializando servidor', error as Error);
    process.exit(1);
  }
};
...
process.on('unhandledRejection', (reason, promise) => {
  logError('💥 Promesa rechazada no manejada', reason as Error, { promise });
  process.exit(1);
});
