...
export const createConversionEvent = async (req, res) => {
    try {
...
export const getConversionEvents = async (req, res) => {
    try {
        const { page = 1, limit = 10, sort = 'desc', sortBy = 'timestamp', eventType, planName, startDate, endDate, sessionId } = req.query;
    ...
    `;
        ...
        logError('Error obteniendo eventos de conversión', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
...
export const getConversionStats = async (req, res) => {
    ...
    `;
        ...
        logError('Error obteniendo estadísticas de conversión', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
...
export const getConversionEventsBySession = async (req, res) => {
    try {
        const { sessionId } = req.params;
        const result = await query('SELECT * FROM conversion_events WHERE session_id = $1 ORDER BY timestamp ASC', [sessionId]);
        const response = {
            success: true,
        ...
        logError('Error obteniendo eventos de conversión por sesión', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
...
export const getConversionEventsByIP = async (req, res) => {
    try {
        const { ip } = req.params;
        const result = await query('SELECT * FROM conversion_events WHERE ip_address = $1 ORDER BY timestamp DESC', [ip]);
        const response = {
            success: true,
        ...
        logError('Error obteniendo eventos de conversión por IP', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
...
export const cleanupOldConversionEvents = async (req, res) => {
    try {
        const { days = 365 } = req.query;
        const result = await query('DELETE FROM conversion_events WHERE created_at < NOW() - INTERVAL $1 DAY', [days]);
        const response = {
            success: true,
        ...
        logError('Error en limpieza de eventos de conversión', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
...
//# sourceMappingURL=conversionEventsController.js.map