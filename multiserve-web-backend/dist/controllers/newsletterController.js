...
export const subscribeToNewsletter = async (req, res) => {
    try {
        ...
        const existingSuscriptor = await query('SELECT * FROM newsletter_suscriptores WHERE email = $1', [data.email]);
    ...
    `;
        ...
        logError('Error creando suscripción al newsletter', error);
        const response = {
            success: false,
...
export const unsubscribeFromNewsletter = async (req, res) => {
    try {
        ...
        const existingSuscriptor = await query('SELECT * FROM newsletter_suscriptores WHERE email = $1', [email]);
        if (existingSuscriptor.rows.length === 0) {
            const response = {
                success: false,
    ...
    `;
        ...
        logError('Error desuscribiendo del newsletter', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
...
export const getNewsletterSubscribers = async (req, res) => {
    try {
        const { page = 1, limit = 10, sort = 'desc', sortBy = 'fecha_suscripcion', estado, fuente, search } = req.query;
    ...
    `;
        ...
        logError('Error obteniendo suscriptores del newsletter', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
...
export const getNewsletterStats = async (req, res) => {
    ...
    `;
        ...
        logError('Error obteniendo estadísticas del newsletter', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
...
export const checkSubscriptionStatus = async (req, res) => {
    try {
        const { email } = req.params;
        const result = await query('SELECT email, estado, fecha_suscripcion, fecha_baja FROM newsletter_suscriptores WHERE email = $1', [email]);
        ...
        logError('Error verificando estado de suscripción', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
...
export const exportNewsletterSubscribers = async (req, res) => {
    try {
        ...
        const result = await query('SELECT email, nombre, fuente, fecha_suscripcion FROM newsletter_suscriptores WHERE estado = $1 ORDER BY fecha_suscripcion DESC', [estado]);
        if (formato === 'csv') {
            // Generar CSV
            const csvHeader = 'Email,Nombre,Fuente,Fecha Suscripción\n';
            const csvRows = result.rows.map((row) => `"${row.email}","${row.nombre || ''}","${row.fuente || ''}","${row.fecha_suscripcion}"`).join('\n');
        ...
        logError('Error exportando lista de suscriptores', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
