...
export const createOrUpdateUserSession = async (req, res) => {
    try {
        ...
        const existingSession = await query('SELECT * FROM user_sessions WHERE session_id = $1', [data.session_id]);
        let session;
      ...
      `;
        ...
        logError('Error procesando sesión de usuario', error);
        const response = {
            success: false,
...
export const getUserSessionById = async (req, res) => {
    try {
        ...
        logError('Error obteniendo sesión de usuario', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
...
export const getUserSessions = async (req, res) => {
    try {
        const { page = 1, limit = 10, sort = 'desc', sortBy = 'first_visit', country, deviceType, isConverted, startDate, endDate } = req.query;
    ...
    `;
        ...
        logError('Error obteniendo sesiones de usuario', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
...
export const updateUserSession = async (req, res) => {
    try {
        ...
        const existingResult = await query('SELECT * FROM user_sessions WHERE session_id = $1', [sessionId]);
    ...
    `;
        ...
        logError('Error actualizando sesión de usuario', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
...
export const getUserSessionStats = async (req, res) => {
    try {
    ...
    `;
        ...
        logError('Error obteniendo estadísticas de sesiones', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
...
export const cleanupOldUserSessions = async (req, res) => {
    try {
        const { days = 365 } = req.query;
        const result = await query('DELETE FROM user_sessions WHERE first_visit < NOW() - INTERVAL $1 DAY', [days]);
        const response = {
            success: true,
        ...
        logError('Error en limpieza de sesiones de usuario', error);
        const response = {
            success: false,
            message: 'Error interno del servidor',
            error: 'INTERNAL_SERVER_ERROR',
...
//# sourceMappingURL=userSessionsController.js.map