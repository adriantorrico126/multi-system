...
BEGIN
    ...
    -- Obtener información de la venta usando mesa_numero en lugar de id_mesa
    SELECT mesa_numero, id_restaurante 
    INTO v_mesa_numero, v_restaurante_id
    FROM ventas 
    WHERE id_venta = p_id_venta;
    ...
    -- Crear registro en historial usando mesa_numero como id_mesa
    INSERT INTO historial_pagos_diferidos (
        ...
        p_id_venta,
        v_mesa_numero,  -- Usar mesa_numero como id_mesa
        v_restaurante_id,
        p_id_pago_final,
...
SELECT 
    ...
    pd.id_venta,
    pd.id_mesa,
    pd.id_mesa as mesa_numero,
    pd.id_restaurante,
    COALESCE(r.nombre, 'Restaurante') as restaurante_nombre,
    pd.total_pendiente,
    pd.fecha_creacion,
    pd.fecha_vencimiento,
    pd.estado,
    pd.observaciones,
    COALESCE(v.fecha, pd.fecha_creacion) as fecha_venta,
    pd.total_pendiente as total_venta,
    CASE 
