-- Script para actualizar el sistema de promociones con funcionalidades avanzadas
...
-- 8. Función para verificar si una promoción es válida en el momento actual
...
-- 9. Función para obtener promociones activas con horarios
...
CREATE OR REPLACE FUNCTION fn_registrar_uso_promocion(
  ...
  p_monto_venta NUMERIC DEFAULT 0
)
RETURNS INTEGER AS $$
DECLARE
...
GROUP BY p.id_promocion, p.nombre, p.tipo, p.valor, p.fecha_inicio, p.fecha_fin, p.estado_promocion, p.destacada, p.segmento_cliente;
