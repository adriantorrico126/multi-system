...
async function corregirTodasColumnasFaltantes() {
  const client = await pool.connect();
  try {
    console.log('🔧 CORRIGIENDO TODAS LAS COLUMNAS FALTANTES EN MOVIMIENTOS_INVENTARIO\n');
    ...
    const columnasNecesarias = [
      ...
      { nombre: 'id_categoria_almacen', tipo: 'INTEGER' }
    ];
    
    for (const columna of columnasNecesarias) {
      const existe = columnasActuales.rows.some(col => col.column_name === columna.nombre);
      ...
      if (!existe) {
        ...
        try {
          ...
          console.log(`  ⚠️ Error agregando ${columna.nombre}: ${error.message}`);
        }
      } else {
    ...
    try {
      ...
      if (productoTest.rows.length > 0) {
        ...
        const movimientoResult = await client.query(`
          SELECT id_movimiento, tipo_movimiento, cantidad, id_categoria_almacen, motivo
    ...
    console.log('\n📋 RESUMEN:');
    console.log('  ✅ Todas las columnas necesarias agregadas a movimientos_inventario');
    ...
    console.error('❌ Error corrigiendo columnas:', error);
  } finally {
    client.release();
    await pool.end();
  }
...
// Ejecutar corrección
corregirTodasColumnasFaltantes()
  .then(() => {
    console.log('\n🏁 Corrección completada');
    process.exit(0);
  })
