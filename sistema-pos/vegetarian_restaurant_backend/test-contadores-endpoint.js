...
async function testContadoresEndpoint() {
  try {
    console.log('🔍 Probando endpoint de contadores...');
    ...
    if (result.rows.length > 0) {
      ...
      console.log('❌ No se encontraron contadores para el restaurante 1');
      
      // Verificar si existen contadores sin JOIN
      const simpleQuery = 'SELECT * FROM contadores_uso WHERE id_restaurante = 1';
      const simpleResult = await pool.query(simpleQuery);
      console.log('Contadores sin JOIN:', simpleResult.rows.length);
      
      // Verificar si existe el plan
      const planQuery = 'SELECT * FROM planes WHERE id_plan = 4';
      const planResult = await pool.query(planQuery);
      console.log('Plan 4 existe:', planResult.rows.length > 0);
    }
