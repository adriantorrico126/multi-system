...
export function SalesHistory({ sales, onDeleteSale, userRole }: SalesHistoryProps) {
  ...
  const filteredSales = sales.filter(sale => {
    ...
    return (
      ...
      sale.tipo_servicio.toLowerCase().includes(searchLower) ||
      (sale.mesa_numero && sale.mesa_numero.toString().includes(searchLower)) ||
      sale.estado.toLowerCase().includes(searchLower) ||
                    ...
                    <TableBody>
                      {filteredSales.map((sale) => (
                        <TableRow key={sale.id}>
                          ...
                          <TableCell>
                            <Badge variant="outline">{sale.tipo_servicio}</Badge>
                          </TableCell>
                          <TableCell>
            ...
            {!showAdvancedAnalytics ? (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  <Card className="hover:shadow-lg transition-shadow cursor-pointer" onClick={() => setShowAdvancedAnalytics(true)}>
            ...
            {selectedSaleForInfo && (
              <>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    <span className="font-medium">Fecha:</span> {formatDate(selectedSaleForInfo.fecha)}
                  </div>
                  <div>
                    <span className="font-medium">Total:</span> {formatCurrency(selectedSaleForInfo.total)}
                  </div>
                  <div>
                    <span className="font-medium">Estado:</span> 
                    <Badge className="ml-2">{selectedSaleForInfo.estado}</Badge>
                  </div>
                  <div>
                    <span className="font-medium">Servicio:</span> {selectedSaleForInfo.tipo_servicio}
                  </div>
                  <div>
