...
UNION ALL
 SELECT 'ventas'::text AS table_name,
    ...
    count(
        CASE
            WHEN ((ventas.estado)::text = ANY (ARRAY[('recibido'::character varying)::text, ('en_preparacion'::character varying)::text, ('listo_para_servir'::character varying)::text, ('abierta'::character varying)::text, ('en_uso'::character varying)::text, ('pendiente_cobro'::character varying)::text])) THEN 1
            ELSE NULL::integer
        END) AS active_mesas,
    count(
        CASE
            WHEN ((ventas.estado)::text = ANY (ARRAY[('entregado'::character varying)::text, ('completada'::character varying)::text, ('pagado'::character varying)::text])) THEN 1
            ELSE NULL::integer
        END) AS free_mesas,
