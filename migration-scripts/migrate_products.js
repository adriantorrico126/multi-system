...
// Ejecutar con: node migrate_products.js
...
async function migrate() {
  try {
    ...
    const backendCatMap = new Map(backendCategories.map(c => [c.nombre, c]));
    ...
    const backendProdNames = new Set(backendProducts.map(p => p.nombre));
    ...
    for (const prod of products) {
      ...
      const cat = backendCatMap.get(prod.category);
      if (!cat) {
