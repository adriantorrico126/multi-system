...
async function fixPlanFunctionalitiesCorrectly() {
  const client = await pool.connect();
  
  try {
    console.log('🔧 Corrigiendo funcionalidades de planes con formato correcto...\n');
    ...
    // Actualizar funcionalidades de cada plan usando el tipo JSONB
    ...
    verifyResult.rows.forEach(row => {
      ...
      try {
        ...
        let funcionalidades;
        if (typeof row.funcionalidades === 'string') {
          funcionalidades = JSON.parse(row.funcionalidades);
        } else {
        ...
        console.log(`   📄 Raw data: ${typeof row.funcionalidades} - ${row.funcionalidades}`);
      }
    });
...
// Ejecutar corrección
fixPlanFunctionalitiesCorrectly().catch(console.error);
